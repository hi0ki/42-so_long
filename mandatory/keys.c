...
void left_right(mlx_key_data_t keydata, mlx *s, int x,int y)
{
    ...
    if (keydata.key == MLX_KEY_D)
        take_thecoins(s, s->player->instances->x / 60, s->player->instances->y / 60, "imgs/to_right.png");
    else if (keydata.key == MLX_KEY_A)
        take_thecoins(s, s->player->instances->x / 60, s->player->instances->y / 60, "imgs/to_left.png");
}
...
void top_bot(mlx_key_data_t keydata, mlx *s, int x, int y)
{
    ...
    if (keydata.key == MLX_KEY_W)
        take_thecoins(s, s->player->instances->x / 60, s->player->instances->y / 60, "imgs/to_top.png");
    else if (keydata.key == MLX_KEY_S)
        take_thecoins(s, s->player->instances->x / 60, s->player->instances->y / 60, "imgs/to_bot.png");
}
